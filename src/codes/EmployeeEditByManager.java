...
public class EmployeeEditByManager implements ActionListener {
	...
	public void showWindow() {
		...
		window.setTitle("Employee Editor");
	...
	public void layoutSetting() {
		...
		panel.setBounds(200, 290, 400, 150);
		panel.setFocusable(true);
		window.add(panel);
		
		panel_1.setBounds(200, 460, 400, 100);
		window.add(panel_1);
		
		JPanel panel_2 = new JPanel(new GridLayout(1,0));
		JLabel message = new JLabel("Empleyee Editor");
		...
		panel_3.add(list);	
		panel_3.setBounds(740, 13, 50, 50);
		window.add(panel_3);
	...
	public void actionPerformed(ActionEvent e) {
		if(e.getSource() == find) {
			...
			try {
				if(id.getText().length() > 0 && title.getText().length() > 0 && post.getText().length() > 0 && salary.getText().length() > 0) {
					pd.update(new Employee(id.getText(), title.getText(), post.getText(), Double.parseDouble(salary.getText())));
					pd.saveToFile();	
			...
			window.dispose();
			ManagerMainWindow d = new ManagerMainWindow();
			d.showWindow();
		}
		
		else if(e.getSource() == list) {
			window.dispose();
			EmployeeListForManager c = new EmployeeListForManager();
			c.showWindow();
		}
